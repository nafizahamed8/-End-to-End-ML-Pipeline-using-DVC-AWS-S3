...
def apply_tf_idf(train_data:pd.DataFrame,test_data:pd.DataFrame,max_features:int):
...
def main():
    try:
        ...
        train_data=load_data('data/interim/train_process_csv')
        test_data=load_data('data/interim/test_process_csv')

        train_df,test_df=apply_tf_idf(train_data,test_data,max_features)
        save_data(train_df,os.path.join("./data","processed","train_tfidf.csv"))
        save_data(test_df,os.path.join("./data","processed","test_tfidf.csv"))
    except Exception as e:
        logger.debug("failed to complete the feature engineering process: %s",e)
        print(f"Error: {e}")
